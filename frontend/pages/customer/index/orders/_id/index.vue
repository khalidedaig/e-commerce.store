  ...
  <loader :loading="$fetchState.pending">
    <div v-if="order">
      <div class="flex justify-between mb-6">
        <nuxt-link
          to="/customer/orders"
          class="inline-flex items-center mb-3 text-gray-900 text-sm hover:text-primary-500"
        >
          <svg
        ...
        <client-only>
          <cancel-order
            :order="order"
            @success="handleCancelRequestSuccess"
      ...
      <!-- order items -->
      <p
        class="flex justify-between items-center font-semibold text-lg mt-10 mb-3"
      >
        <span>Order Items</span>
        <button
          class="btn btn-success"
          :class="{ 'opacity-50': isRequestProcessing }"
      ...
      <div class="rounded-md overflow-hidden shadow-sm border">
            ...
            <tbody>
              <tr v-for="(item, index) in order.orderItems" :key="index">
                ...
                <td>
                  ...
                  <span v-else> {{ item.quantity }} {{ item.unit.code }} </span>
                </td>
                <td class="text-right font-medium">
                  {{ currencyFormat(item.quantity * item.rate) }}
                </td>
                <td class="text-right">
                  <remove-icon-button
