  ...
  <div>
    ...
    <div class="flex flex-col md:flex-row justify-between md:items-center mb-4">
      ...
      <div class="flex justify-between mt-4 md:mt-0">
        ...
        <action-dropdown
          v-if="!disableCheck"
          :align="hasFilterSlot || hasButtonSlot ? 'left' : 'right'"
          :class="{ 'mr-4': hasFilterSlot || hasButtonSlot }"
        >
          <div>
            <slot
            ...
            <div v-else class="text-sm tracking-wide text-center">
              No items selected
            </div>
          </div>
        </action-dropdown>
        ...
        <!-- Filter Dropdown -->
        <filter-dropdown
          :align="hasButtonSlot ? 'left' : 'right'"
          v-if="hasFilterSlot"
          :class="{ 'mr-4': hasButtonSlot }"
        >
          <div class="px-1 py-2">
                      ...
                      <div class="flex justify-end items-center">
                        ...
                        <div v-else>
                          ...
                          <button
                            ...
                            @click="forceDelete(row.id, index)"
                          >
                            <i class="ti-eraser"></i>
                          </button>
                        </div>
    ...
    <div
      class="flex flex-col sm:flex-row justify-between items-center mt-6 invisible md:visible"
      v-if="laravelData.data && laravelData.data.length"
    >
      <div class="text-gray-600 text-sm mb-4 sm:mb-0">
        Showing {{ laravelData.meta.from }}-{{ laravelData.meta.to }} of
        {{ laravelData.total }} data
      </div>
      <!-- Table Pagination -->
      <pagination
        :limit="3"
    ...
    <div
      class="flex flex-col sm:flex-row justify-between items-center mt-6 visible md:invisible"
      v-if="laravelData.data && laravelData.data.length"
    >
      <div class="text-gray-600 text-sm mb-4 sm:mb-0">
        Showing {{ laravelData.meta.from }}-{{ laravelData.meta.to }} of
